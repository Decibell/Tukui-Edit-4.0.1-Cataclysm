	...
	elseif self:GetAnchorType() == "ANCHOR_NONE" then
		if InCombatLockdown() and db.hidecombat == true then
			...
			if TukuiCF["bags"].enable == true and StuffingFrameBags:IsShown() then
				self:ClearAllPoints()
				self:SetPoint("BOTTOMRIGHT", StuffingFrameBags, "TOPRIGHT", 0, 3)
			elseif HasPetUI() then
				if TukuiCF["actionbar"].rightbars_vh == true then
					self:ClearAllPoints()
					self:SetPoint("BOTTOMRIGHT", PetActionButton10, "BOTTOMLEFT", -3, 0)
				else
					self:ClearAllPoints()
					self:SetPoint("BOTTOMRIGHT", PetActionButton10, "TOPRIGHT", 0, 3)
				end
			elseif UnitHasVehicleUI("player") then
				self:ClearAllPoints()
				self:SetPoint("BOTTOMRIGHT", TukuiChatRightTabs, "TOPRIGHT", 0, 3)
			else
				if TukuiCF["actionbar"].rightbars_vh == true then
					if TukuiCF["actionbar"].rightbars == 0 then
						self:ClearAllPoints()
						self:SetPoint("BOTTOMRIGHT", TukuiChatRightTabs, "BOTTOMLEFT", -3, 0)
					elseif TukuiCF["actionbar"].rightbars == 1 then
						self:ClearAllPoints()
						self:SetPoint("BOTTOMRIGHT", MultiBarRightButton12, "BOTTOMLEFT", -3, 0)
					elseif TukuiCF["actionbar"].split_bar == true or TukuiCF["actionbar"].rightbars == 2 then
						self:ClearAllPoints()
						self:SetPoint("BOTTOMRIGHT", MultiBarBottomRightButton12, "BOTTOMLEFT", -3, 0)
					elseif (not TukuiCF["actionbar"].split_bar == true) and TukuiCF["actionbar"].rightbars == 3 then	
						self:ClearAllPoints()
						self:SetPoint("BOTTOMRIGHT", MultiBarLeftButton12, "BOTTOMLEFT", -3, 0)
					end
				elseif not TukuiCF["actionbar"].rightbars_vh == true then
					if TukuiCF["actionbar"].rightbars == 0 then
						self:ClearAllPoints()
						self:SetPoint("BOTTOMRIGHT", TukuiChatRightTabs, "TOPRIGHT", 0, 3)
					elseif TukuiCF["actionbar"].rightbars == 1 then
						self:ClearAllPoints()
						self:SetPoint("BOTTOMRIGHT", MultiBarRightButton12, "TOPRIGHT", 0, 3)
					elseif TukuiCF["actionbar"].split_bar == true or TukuiCF["actionbar"].rightbars == 2 then
						self:ClearAllPoints()
						self:SetPoint("BOTTOMRIGHT", MultiBarBottomRightButton12, "TOPRIGHT", 0, 3)
					elseif (not TukuiCF["actionbar"].split_bar == true) and TukuiCF["actionbar"].rightbars == 3 then	
						self:ClearAllPoints()
						self:SetPoint("BOTTOMRIGHT", MultiBarLeftButton12, "TOPRIGHT", 0, 3)
					end
				else
					self:ClearAllPoints()
					self:SetPoint("BOTTOMRIGHT", TukuiChatRightTabs, "TOPRIGHT", 0, 3)
...
TukuiDB.StyleShadow(healthBarBG)
...
TukuiTooltip:SetScript("OnEvent", function(self)
	...
	TukuiDB.StyleShadow(FriendsTooltip)
	TukuiDB.StyleShadow(GameTooltip)
