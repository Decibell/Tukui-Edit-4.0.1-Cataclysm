...
local function ConfigureBar(f)
	...
	f.Bar:SetStatusBarTexture(TukuiCF["theme"].UI_Texture)
...
local WeakenedSoulLoader = CreateFrame("Frame", nil, UIParent)

WeakenedSoulLoader:RegisterEvent("ADDON_LOADED")
WeakenedSoulLoader:SetScript("OnEvent", function(self, event, addon)
	if not (addon == "ecUnitframes" or addon == "Tukui_HealMode") then return end
